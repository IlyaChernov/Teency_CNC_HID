...
class QueueArray {
  ...
  private:
    ...
    static const long initialSize = 2;
    ...
    long size;        // the size of the queue.
    long items;       // the number of items of the queue.

    long head;        // the head of the queue.
    long tail;        // the tail of the queue.
};
...
// resize the size of the queue.
template<typename T>
void QueueArray<T>::resize (const long s) {
  // defensive issue.
  if (s <= 0)
